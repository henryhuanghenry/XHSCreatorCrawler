...
class AsyncFileWriter:
    def __init__(self, platform: str, crawler_type: str, creator_id: str = ""):
    ...
    def _get_file_path(self, file_type: str, item_type: str) -> str:
        if config.SAVE_DATA_PATH:
            base_path = f"{config.SAVE_DATA_PATH}/{self.platform}"
        else:
            base_path = f"data/{self.platform}"
        
        # If creator_id is set and crawler_type is "creator", add creator subdirectory
        if self.creator_id and self.crawler_type == "creator":
            base_path = f"{base_path}/creator_{self.creator_id}"
        
        base_path = f"{base_path}/{file_type}"
        ...
        async with self.lock:
            ...
            if os.path.exists(file_path) and os.path.getsize(file_path) > 0:
                try:
                    ...
                    # If file is corrupted or has encoding issues, backup and start fresh
                    utils.logger.error(f"[AsyncFileWriter] Error reading {file_path}: {e}")
                    backup_path = f"{file_path}.corrupted.{int(time.time())}"
                    try:
                        import shutil
                        shutil.copy2(file_path, backup_path)
                        utils.logger.warning(f"[AsyncFileWriter] Corrupted file backed up to: {backup_path}")
                    except Exception as backup_error:
                        utils.logger.error(f"[AsyncFileWriter] Failed to backup corrupted file: {backup_error}")
                    existing_data = []
