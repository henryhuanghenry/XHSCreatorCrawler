...
async def _export_to_markdown_if_needed() -> None:
    """Export crawled data to markdown files"""
    if config.SAVE_DATA_OPTION != "json":
        return
    
    try:
        from tools.markdown_exporter import export_latest_crawl_results
        
        print("[Main] Exporting crawled data to markdown files...")
        created_files = await export_latest_crawl_results(platform=config.PLATFORM)
        
        if created_files:
            print(f"[Main] Successfully exported {len(created_files)} notes to markdown")
            print(f"[Main] Markdown files location: data/{config.PLATFORM}/markdown_export/")
        else:
            print("[Main] No data to export")
    except Exception as e:
        print(f"[Main] Error exporting to markdown: {e}")
...
async def main() -> None:
    ...
    await _export_to_markdown_if_needed()
