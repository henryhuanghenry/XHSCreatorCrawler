...
# Date window for creator crawling (YYYY-MM-DD format)
# If set, only crawl notes within this date range
# Leave empty to crawl all notes up to CRAWLER_MAX_NOTES_COUNT
CRAWLER_START_DATE = ""  # Start date (inclusive), e.g., "2024-01-01"
CRAWLER_END_DATE = ""    # End date (inclusive), e.g., "2024-12-31"

# Creator crawling mode configuration (for CRAWLER_TYPE = "creator")
# "full": Crawl all note IDs from scratch, ignoring existing records
# "incremental": Only crawl new note IDs, stop when encountering many existing IDs
CREATOR_CRAWL_MODE = "incremental"  # "full" or "incremental" full

# Incremental mode stop threshold (0.0-1.0)
# If this ratio of fetched IDs already exists, stop crawling in incremental mode
# E.g., 0.5 means stop if 50% or more IDs already exist in a single page
INCREMENTAL_STOP_THRESHOLD = 0.5

# Note fetch order configuration (for CRAWLER_TYPE = "creator", phase 2)
# "random": Fetch notes in random order (helps avoid anti-crawler detection)
# "sorted": Fetch notes in descending order by note_id (larger IDs first)
CREATOR_NOTE_FETCH_ORDER = "sorted"  # "random" or "sorted"

# Whether to fetch notes marked as not_found (deleted/unavailable)
# When False, notes that returned "not found" error will be skipped in future crawls
FETCH_NOT_FOUND_NOTES = False
