...
async def parse_cmd(argv: Optional[Sequence[str]] = None):
    ...
    def main(
        ...
        enable_get_media: Annotated[
            ...
            typer.Option(
                "--enable_get_media",
                help="Whether to enable media crawling (images), supports yes/true/t/y/1 or no/false/f/n/0",
                rich_help_panel="Basic Configuration",
                show_default=True,
            ),
        ...
        start_date: Annotated[
            ...
            typer.Option(
                "--start_date",
                help="Start date for creator crawling (YYYY-MM-DD), e.g., 2024-01-01",
                rich_help_panel="Creator Configuration",
            ),
        ] = config.CRAWLER_START_DATE,
        end_date: Annotated[
            str,
            typer.Option(
                "--end_date",
                help="End date for creator crawling (YYYY-MM-DD), e.g., 2024-12-31",
                rich_help_panel="Creator Configuration",
            ),
        ] = config.CRAWLER_END_DATE,
    ) -> SimpleNamespace:
        """MediaCrawler 命令行入口"""
        ...
        enable_get_media_value = _to_bool(enable_get_media)
        ...
        config.ENABLE_GET_MEIDAS = enable_get_media_value
        config.CRAWLER_START_DATE = start_date
        config.CRAWLER_END_DATE = end_date
